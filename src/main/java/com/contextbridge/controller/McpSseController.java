 ...
 *
 * <p>Implements a lightweight Model Context Protocol transport over Server-Sent Events:
 * <ul>
 *   <li>{@code GET  /mcp/sse}              — opens an SSE stream; IDE agents connect here.</li>
 *   <li>{@code POST /mcp/checkpoint_state} — IDE agent sends a snapshot; we embed + persist it.</li>
 *   <li>{@code POST /mcp/restore_state}    — IDE agent requests the best snapshot for a project.</li>
 * </ul>
 *
 * <p>The SSE stream pushes a {@code tool_definitions} event on connect so that compliant
...
public class McpSseController {
    ...
    /**
     * {@code restore_state} tool — retrieves the most relevant snapshot for a project.
     *
        ...
        return Map.of(
                ...
                "tools", java.util.List.of(
                        Map.of(
                                ...
                                "description", "Summarize current working context into a snapshot and persist it.",
                                "input_schema", Map.of(
                                        "type", "object",
                                        "required", java.util.List.of(
                                                "project_name", "session_id", "current_goal"),
                                        "properties", Map.of(
                                                "project_name",            Map.of("type", "string"),
                                                "session_id",              Map.of("type", "string"),
                                                "current_goal",            Map.of("type", "string"),
                                                "active_files",            Map.of("type", "array", "items", Map.of("type", "string")),
                        ...
                        Map.of(
                                "name",        "restore_state",
                                "description", "Retrieve the most relevant context snapshot for a project.",
                                "input_schema", Map.of(
                                        "type", "object",
                                        "required", java.util.List.of("project_name"),
                                        "properties", Map.of(
                                                "project_name", Map.of("type", "string")
                                        )
                                )
